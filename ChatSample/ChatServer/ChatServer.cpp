...
// there is a common bug is the SDK headers and libs that
// causes IID_IAccessControl to be undefined.We define it here
// to give the GUID linkage.
DEFINE_GUID(IID_IAccessControl, 0xEEDD23E0, 0x8410, 0x11CE,
	0xA1, 0xC3, 0x0B, 0x00, 0x2B, 0x2B, 0x8D, 0x8F);
#endif
...
const char* g_RegTable[][3] = {
	{ "Software\\Classes\\CLSID\\{5223A053-2441-11d1-AF4F-0060976AA886}", 0, "ChatSession" },
	{ "Software\\Classes\\CLSID\\{5223A053-2441-11d1-AF4F-0060976AA886}", "Appld", "{5223A054-2441-11dl-AF4F-0060976M886}"} ,
	{ "Software\\Classes\\CLSID\\{5223A053-2441-11d1-AF4F-0060976AA886}\\LocalServer32",0, (const char*)-1 // rogue value indicating file name
	} ,
	{ "Software\\Classes\\AppID\\{5223A054-2441-11d1-AF4F-0060976AA886}",0, "ChatSession Server" },
	{ "Software\\Classes\\AppID\\{5223A054-2441-11d1-AF4F-0060976AA886}","RunAs", "Domain\\ReplaceMe"} ,
	{ "Software\\Classes\\AppID\\{5223A054-2441-11d1-AF4F-0060976AA886}","Chat Admins Group", "Domain\\ReplaceMe"} ,
	{ "Software\\Classes\\AppID\\{5223A054-2441-11d1-AF4F-0060976AA886}","Chat Users Group", "Domain\\ReplaceMe"} ,
	{ "Software\\Classes\\AppID\\COMChat.exe","Appld", "{5223A054-2441-11d1-AF4F-0060976AA886}"
	} ,
};
...
STDAPI UnregisterServer(void) {
	...
	for (int i = nEntries - 1; i >= 0; i--) {
		long err = RegDeleteKeyA(HKEY_CURRENT_USER, g_RegTable[i][0]);
...
STDAPI RegisterServer(HINSTANCE hInstance = 0) {
	...
	for (int i = 0; SUCCEEDED(hr) && i < nEntries; i++) {
		...
		HKEY hkey;
		// create the key
		long err = RegCreateKeyA(HKEY_CURRENT_USER,
			pszKeyName, &hkey);
		if (err == ERROR_SUCCESS) {
			// set the value
...
// this routine is called at process lnlt time
...
HRESULT InitializeApplicationSecurity(void) {
	...
	long err = RegOpenKeyExW(HKEY_CLASSES_ROOT,
		L"AppID\\{5223A054-2441-11d1-AF4F-0060976AA886}",
		0, KEY_QUERY_VALUE, &hkey);
	if (err == ERROR_SUCCESS) {
		DWORD cb = sizeof(wszAdminsGroup);
		err = RegQueryValueExW(hkey, L"Chat Admins Group",
			0, 0, (BYTE*)wszAdminsGroup, &cb);
		cb = sizeof(wszAdminsGroup);
		if (err == ERROR_SUCCESS)
			err = RegQueryValueExW(hkey, L"Chat Users Group", 0, 0,
				(BYTE*)wszUsersGroup, &cb);
		RegCloseKey(hkey);
	}
	if (err != ERROR_SUCCESS)
	...
	// initialize process - wide security
	...
	if (SUCCEEDED(hr)) {
		...
		hr = CoCreateInstance(CLSID_DCOMAccessControl, 0, CLSCTX_ALL,
			IID_IAccessControl, (void**)&g_pacUsers);
		if (SUCCEEDED(hr))
			hr = g_pacUsers->SetAccessRights(&aaUsers);
		if (SUCCEEDED(hr)) {
			hr = CoCreateInstance(CLSID_DCOMAccessControl, 0, CLSCTX_ALL,
				IID_IAccessControl, (void**)&g_pacAdmins);
			if (SUCCEEDED(hr))
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR szCmdParam, int) {
	const wchar_t* pszPrompt =
		L"Ensure that you have properly configured the "
		L"application to run as a particular user and that "
		L" you have manually changed the Users and Admins "
		L"Group registry settings under this server's AppID.";
	...
	if (strstr(szCmdParam, "/UnregServer") != 0
		...
		hr = RegisterServer();
		MessageBoxW(0, pszPrompt, L"COMChat", MB_SETFOREGROUND);
		CoUninitialize();
		return hr;
	}
	...
	if (FAILED(hr))
		MessageBoxW(0, pszPrompt, L"Error", MB_SETFOREGROUND);
	CoUninitialize();
	return 0;
}