...
#include <iaccess.h>
// these server lifetime control routines are defined in svc.cpp
extern void ModuleLock(void);
extern void ModuleUnlock(void);
// these access control objects are created in svc.cpp to control
// various privileged operations.Most operations in this class
// are non - privileged, so anyone can get in.
extern IAccessControl* g_pacUsers;
extern IAccessControl* g_pacAdmins;
// utility functions IIIIIIIIIIIIIIIIIIIIIIIII
// duplicate an OLECHAR* using CoTaskMemAlloc
OLECHAR* OLESTRDUP(const OLECHAR* pwsz) {
	auto cb = sizeof(OLECHAR) * (wcslen(pwsz) + 1);
...
OLECHAR* GetCaller(void) {
	OLECHAR* pwsz = 0;
	HRESULT hr =
		CoQueryClientBlanket(0, 0, 0, 0, 0, (void**)&pwsz, 0);
	if (SUCCEEDED(hr))
...
// class ChatSession ///////////////////////////////
ChatSession::ChatSession(const OLECHAR * pwszSessionName, bool bAllowAnonymousAccess):
	m_cRef(0),
	m_csStatementLock{},
	m_csAdviseLock{},
	m_wszSessionName{},
...
void ChatSession::Disconnect(void) {
	(void*)CoDisconnectObject(this, 0);
	// tear down connected listeners
	ALock();
	while (m_pHeadListeners) {
		LISTENER* pThisNode = m_pHeadListeners;
...
void ChatSession::Fire_OnNewStatement(const OLECHAR* pwszUser,
...
void ChatSession::Fire_OnNewUser(const OLECHAR* pwszUser) {
	ALock();
	for (LISTENER* pNode = m_pHeadListeners;
		pNode != 0; pNode = pNode->pNext)
		if (pNode->pItf)
...
void ChatSession::Fire_OnUserLeft(const OLECHAR* pwszUser) {
	ALock();
	for (LISTENER* pNode = m_pHeadListeners;
		pNode != 0; pNode = pNode->pNext)
		if (pNode->pItf)
...
bool ChatSession::CheckAccess(const OLECHAR* pwszUser) {
...
STDMETHODIMP_(ULONG) ChatSession::AddRef(void) {
	ModuleLock();
	return InterlockedIncrement(&m_cRef);
}

STDMETHODIMP_(ULONG) ChatSession::Release(void) {
...
STDMETHODIMP ChatSession::get_SessionName(OLECHAR** ppwsz) {
	if (!ppwsz)
		return E_INVALIDARG;
...
STDMETHODIMP ChatSession::Say(const OLECHAR* pwszStatement) {
	HRESULT hr = S_OK;
...
STDMETHODIMP ChatSession::GetStatements(IEnumString** ppes) {
	if (ppes == 0)
		return E_INVALIDARG;
	if ((*ppes = new StatementEnumerator(this)) == 0)
		return E_OUTOFMEMORY;
	(*ppes)->AddRef();
	return S_OK;
}
...
STDMETHODIMP
ChatSession::Advise(IChatSessionEvents* pEventSink, DWORD* pdwReg)
{
	HRESULT hr = S_OK;
	...
	// return node pointer as cookie
	*pdwReg = reinterpret_cast<DWORD>(pNew);
	return hr;
}

STDMETHODIMP ChatSession::Unadvise(DWORD dwReg) {
	if (dwReg == 0)
		return E_INVALIDARG;
	HRESULT hr = S_OK;
	// get node ptr from cookie
	...
	delete pThisNode;
	AUnlock();
	// notify other users this caller has left
	Fire_OnUserLeft(pwszUser);
	CoTaskMemFree(pwszUser);
	return hr;
}

// class StatementEnumerator IIIIIIIIIIIIIIIIIII
StatementEnumerator::StatementEnumerator(ChatSession* pThis)
	: m_cRef(0),
	m_pThis(pThis), m_cursor(pThis->m_statements.begin())
{
	m_pThis->AddRef();
...
// lock helpers(note that ChatSession is locked simultaneously)
void StatementEnumerator::Lock(void) {
	EnterCriticalSection(&m_csLock);
	m_pThis->SLock();
}

void StatementEnumerator::Unlock(void) {
...
STDMETHODIMP_(ULONG) StatementEnumerator::AddRef(void) {
	return InterlockedIncrement(&m_cRef);
}

STDMETHODIMP_(ULONG) StatementEnumerator::Release(void) {
	LONG res = InterlockedDecrement(&m_cRef);
	if (res == 0)
		delete this;
	return res;
...
// IEnumString methods
STDMETHODIMP StatementEnumerator::Next(ULONG cElems,
	OLECHAR** rgElems, ULONG* pcFetched) {
...
STDMETHODIMP StatementEnumerator::Skip(ULONG cElems) {
	Lock();
	ULONG cActual = 0;
	while (cActual < cElems
		&& m_cursor != m_pThis->m_statements.end()) {
...
STDMETHODIMP StatementEnumerator::Reset(void) {
	Lock();
	m_cursor = m_pThis->m_statements.begin();
	Unlock();
	return S_OK;
}
STDMETHODIMP StatementEnumerator::Clone(IEnumString** ppes) {
	if (ppes == 0)
		return E_INVALIDARG;
	if ((*ppes = new StatementEnumerator(m_pThis)) == 0)
		return E_OUTOFMEMORY;
	(*ppes)->AddRef();
	return S_OK;
}
// class ChatSessionClass IIIIIIIIIIIIIIIIIIIII
...
void ChatSessionClass::Lock(void)
{
	EnterCriticalSection(&m_csSessionLock);
}

void ChatSessionClass::Unlock(void)
{
...
bool ChatSessionClass::CheckAccess(const OLECHAR* pwszUser) {
	if (wcscmp(pwszUser, L"anonymous") == 0)
		return false;
	TRUSTEEW trustee = {
	0, NO_MULTIPLE_TRUSTEE, TRUSTEE_IS_NAME,
...
STDMETHODIMP_(ULONG) ChatSessionClass::AddRef(void) {
	return 2;
}

STDMETHODIMP_(ULONG) ChatSessionClass::Release(void) {
	return 1;
}
...
ChatSessionClass::ReleaseConnection(DWORD extconn, DWORD,
	...
	if (extconn & EXTCONN_STRONG) {
		...
		if (res == 0 && blastReleaseKillsStub)
			(void*)CoDisconnectObject(static_cast<IExternalConnection*>(this),0);
		ModuleUnlock();
		return res;
	}
	return 0;
...
ChatSessionClass::GetSessionNames(IEnumString** ppes) {
	if (ppes == 0)
		return E_INVALIDARG;
	if ((*ppes = new SessionNamesEnumerator(this)) == 0)
		return E_OUTOFMEMORY;
	(*ppes)->AddRef();
	return S_OK;
}
	...
	IChatSession** ppcs)
{
	...
	*ppcs = 0;
	...
	if (it == m_sessions.end()) {
		if (bDontCreate)
			hr = E_FAIL;
		else if (!bAllowAnonymousAccess && wcscmp(pwszUser, L"anonymous") == 0)
			hr = E_ACCESSDENIED;
		else {
			ChatSession* pNew =
			...
			if (pNew) {
				pNew->AddRef();
				m_sessions.insert(
					std::pair<std::wstring, ChatSession*>(pwszSessionName, pNew));
				(*ppcs = pNew)->AddRef();
				hr = S_OK;
			}
			else
...
SessionNamesEnumerator::SessionNamesEnumerator(
	...
	m_cRef{ 0 }, m_pStrings{ new std::vector<std::wstring> }, m_pCloneSource{ 0 }
{
	typedef ChatSessionClass::SESSIONMAP SESSIONMAP;
	typedef ChatSessionClass::SESSIONMAP::iterator iterator;
	SESSIONMAP& sessions = pSessionClass->m_sessions;
	// take snapshot of strings from session class
	pSessionClass->Lock();
...
SessionNamesEnumerator::SessionNamesEnumerator(
	...
	m_cRef(0), m_pStrings(0), m_pCloneSource(pCloneSource)
{
...
std::vector<std::wstring>& SessionNamesEnumerator::Strings(void) {
	if (m_pStrings)
		return *m_pStrings;
	else
...
void SessionNamesEnumerator::Lock(void)
{
	EnterCriticalSection(&m_csLock);
}

void SessionNamesEnumerator::Unlock(void)
{
	LeaveCriticalSection(&m_csLock);
}
...
STDMETHODIMP_(ULONG) SessionNamesEnumerator::AddRef(void) {
	ModuleLock();
	return InterlockedIncrement(&m_cRef);
}
STDMETHODIMP_(ULONG) SessionNamesEnumerator::Release(void) {
	LONG res = InterlockedDecrement(&m_cRef);
	if (res == 0)
		delete this;
	ModuleUnlock();
...
SessionNamesEnumerator::Next(ULONG cElems, OLECHAR** rgElems,
	...
	ULONG cActual = 0;
	Lock();
	while (cActual < cElems && m_cursor != Strings().end()) {
		if (rgElems[cActual] = OLESTRDUP((*m_cursor).c_str())) {
			m_cursor++;
			cActual++;
		}
...
STDMETHODIMP SessionNamesEnumerator::Skip(ULONG cElems) {
	ULONG cActual = 0;
	Lock();
	while (cActual < cElems && m_cursor != Strings().end()) {
		m_cursor++;
...
STDMETHODIMP SessionNamesEnumerator::Reset(void) {
	Lock();
	m_cursor = Strings().begin();
	Unlock();
	return S_OK;
...
STDMETHODIMP SessionNamesEnumerator::Clone(IEnumString** ppes) {
	if (ppes == 0)
		return E_INVALIDARG;
	SessionNamesEnumerator* pCloneSource = m_pCloneSource;
	if (pCloneSource == 0) // we are the source
		m_pCloneSource = this;
	if ((*ppes = new SessionNamesEnumerator(pCloneSource)) == 0)
		return E_OUTOFMEMORY;
	(*ppes)->AddRef();
	return S_OK;
}